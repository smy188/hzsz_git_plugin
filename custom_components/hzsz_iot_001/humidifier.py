"""Humidifier entities for Milesight IoT devices — v2.0 entity-centric.

A humidifier aggregates properties with roles:
  power, current_humidity, target_humidity, mode
"""
...
async def async_setup_entry(hass: HomeAssistant, config_entry: HzszConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    for device in hub.all_devices():
        entities.extend(_build_humidifiers(device))
    async_add_entities(entities)

    @callback
    def _on_new_device(device: DynamicDevice) -> None:
        new_entities = _build_humidifiers(device)
        if new_entities:
            async_add_entities(new_entities)
    config_entry.async_on_unload(async_dispatcher_connect(hass, SIGNAL_NEW_DEVICE, _on_new_device))


def _build_humidifiers(device: DynamicDevice) -> list["PushedHumidifier"]:
    result: list[PushedHumidifier] = []
    for entity_def in device.get_entities_by_type("humidifier"):
        result.append(PushedHumidifier(device, entity_def))
    return result


class PushedHumidifier(HumidifierEntity):
    should_poll = False
    ...
    def __init__(self, device: DynamicDevice, entity_def: dict) -> None:
        ...
        # Device class (humidifier / dehumidifier)
        dc = config.get("deviceClass")
        if dc:
            try:
                self._attr_device_class = HumidifierDeviceClass(dc)
            except ValueError:
                pass

        self._attr_min_humidity = float(config.get("minHumidity", 0))
        self._attr_max_humidity = float(config.get("maxHumidity", 100))
        modes = config.get("availableModes", [])
        self._attr_available_modes = modes if modes else None

        self._attr_supported_features = HumidifierEntityFeature(0)
        if "target_humidity" in self._props:
    ...
    def current_humidity(self) -> int | None:
        raw = self._get_raw("current_humidity")
        if raw is not None:
            try:
    ...
    def target_humidity(self) -> int | None:
        raw = self._get_raw("target_humidity")
        if raw is not None:
            try:
                return int(float(raw))
            except (ValueError, TypeError):
    ...
    async def async_set_humidity(self, humidity: int) -> None:
        self._publish_cmd("target_humidity", str(humidity))
